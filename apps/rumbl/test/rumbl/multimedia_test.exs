defmodule Rumbl.MultimediaTest do
  ...
  describe "categories" do
    test "list_alphabetical_categories/0" do
      ...
      assert alpha_names == ["A", "B", "C"]
    end
  end

  describe "videos" do
    @valid_attrs %{description: "descr", title: "title", url: "http://local"}
    ...
    test "create_video/2 with valid data creates a video" do
      ...
      assert video.description == "descr"
    ...
    test "update_video/2 with valid data updates the video" do
      ...
      assert %Video{id: id, title: "updated title"} = video
      assert %Video{id: ^id, title: "updated title"} = Multimedia.get_video!(id)
    end
